...
<div class="docs_block">
  <Button iconStart={<Sun />}>Icon start</Button>
  <Button iconEnd={<Moon />}>Icon end</Button>
</div>
...
```jsx
<Button iconStart={{/* insert icon */}}>Icon start</Button>
<Button iconEnd={{/* insert icon */}}>Icon end</Button>
```
  </TabItem>
  <TabItem value="css" label="CSS">
...
By default, buttons have `type="button"` to prevent unintended form submissions. Override with `type="submit"` or `type="reset"` when needed.
